...
| 자동 생성 썸네일 (`scripts/gen-thumbnail.mjs`) | 하드 섀도우·배지·장식 아이콘에 사용 |
...
6. **본문 가독성이 배경 무늬보다 우선한다.** `body`에 `/textures/bg-pattern.svg`(노트북·코드 브래킷·터미널·칩 라인 아이콘, 220px 타일, 불투명도 6~7%)가 깔려 있다. `.prose`에는 불투명한 흰 배경을 덮어 글자 위로 무늬가 겹치지 않게 했다 — 이 규칙을 지운 채 무늬를 진하게 만들지 않는다.
...
| `PostHero.astro` | 글 상단 제목 박스 (카테고리 배지 · 제목 · 설명 · 날짜 · 태그). **썸네일 이미지는 넣지 않는다** |
| `PostCard.astro` | 목록 카드. `showCategory` prop으로 카테고리 배지를 끌 수 있다 (홈 카테고리 섹션에서 사용). 여기에도 썸네일 이미지는 넣지 않는다 |
...
- 섹션 구분선은 **콘텐츠 폭 안에만** 그린다. 전체 폭 배경 밴드가 필요하면 바깥 `<section>`에 배경을, 안쪽 `max-w-6xl` 컨테이너에 `border-y`를 준다.
